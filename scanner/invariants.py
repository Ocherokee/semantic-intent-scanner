...
INVARIANTS = [
    ...
    },
    {
        ...
        "rationale": (
            ...
            "'External source' is not limited to developer artefacts. It includes "
            "ordinary operational communications arriving through channels the agent "
            "monitors — email, tickets, chat, webhooks. A situation report that the "
            "agent turns into a derived action ('renewal payment failed' -> pay it) "
            "is covered as much as an explicit instruction: the trust decision is "
            "still owed. Three checks stay separate: understanding the message is "
            "not verifying its claim is true, and verifying the claim is not "
            "establishing that the sender may act on it. They are orthogonal — a "
            "true claim confers no authority, so authority verification is not a "
            "fallback for when truth verification fails. Goal-fit is not "
            "authorisation — that an action would advance a standing objective is "
            "not evidence that a party with authority asked for it. An authenticated "
            "transport is not an authenticated identity — SPF/DKIM/DMARC passing for "
            "a sending domain says nothing about whether that sender is who it claims "
            "to be or may request what it requests. "
            "See docs/semantic-failure-patterns.md (SFP-1, Workflow Injection)."
        ),
        "signal_patterns": [
            "before responding",
            "before you answer",
